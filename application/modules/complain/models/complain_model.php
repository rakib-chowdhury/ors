...
public function get_somitee_num($type)
    {
        ...
        if ($type == 'new') {            
            $this->db->where('complain_reply',null);            
            $this->db->where('complain_reply_date','0000-00-00');            
        }
        if ($type == 'reply') {           
            $this->db->where_not_in('complain_reply',null);            
            $this->db->where_not_in('complain_reply_date','0000-00-00');             
        }
 ...
 /*public function get_somitee_num($type)
    {
        ...
        if ($type == 'reply') {           
            $this->db->where_not_in('somitee_status', 3);
...
public function get_complain($complain_id)
    ...
    public function get_complain_info($somitee_id)
    {
        $this->db->select('*');
        $this->db->from('complain');
...
public function update_complain($c_id, $data)
    {
        $this->db->where('complain_id', $c_id);
 ...
 public function get_all_admin_info($id,$tbl){
    ...
    public function get_somitee($type)
    {
        $this->db->select('*');
        $this->db->from('complain');
        $this->db->join('somitee_info', 'complain.somitee_id=somitee_info.somitee_id','left');
        //$this->db->where_not_in('somitee_info.complain_id', 0);
        if ($type == 'all') {
            $this->db->order_by('complain.complain_date', 'desc');
        }
        if ($type == 'new') {
            $this->db->where('complain.complain_reply', null);
            $this->db->order_by('complain.complain_date', 'desc');
        }
        if ($type == 'reply') {//replied
            $this->db->where_not_in('complain.complain_reply', null);
            $this->db->where_not_in('complain.complain_reply_date', '0000-00-00');
            $this->db->order_by('complain_reply_date', 'desc');
        }
